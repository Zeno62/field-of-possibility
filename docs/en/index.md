# English Edition  
## Field of Possibility

The English edition of **Field of Possibility / 可能性网络** is currently in progress.

The Chinese edition is the source edition of this project.

Current source version:

**v4.0.0 Official Chinese Edition**

---

## Translation Status

Status:

**Draft preparation**

The English version has not yet been finalized.

Before the full English white paper is released, the project will first maintain:

- a terminology map;
- a translation workflow;
- a short English introduction;
- selected translated chapters;
- reviewed translation proposals.

---

## Source Edition

The source edition is:

**可能性网络白皮书 v4.0 正式版**

Read the Chinese source edition:

- <a href="https://zeno62.github.io/field-of-possibility/zh-Hans/">Chinese Source Edition</a>
- <a href="https://github.com/Zeno62/field-of-possibility/blob/main/whitepaper/zh-Hans/full/field-of-possibility-whitepaper-v4.0-zh-Hans.md">Full Chinese Markdown on GitHub</a>
- <a href="https://github.com/Zeno62/field-of-possibility/releases/tag/v4.0.0">Release v4.0.0</a>

---

## Translation Principles

The English edition should preserve:

- the distinction between Awareness and Consciousness;
- the distinction between Pure Possibility and Field of Possibility;
- the meaning of **认假为真**;
- the meaning of Single-Frame Interruption;
- the non-religious and non-self-help boundary of the project.

The English edition should not turn this framework into:

- a religion;
- a mystical system;
- a self-help doctrine;
- a success philosophy;
- a quantum mysticism framework.

---

## Key Files

- <a href="https://github.com/Zeno62/field-of-possibility/blob/main/glossary/translation-map.md">Translation Map</a>
- <a href="https://github.com/Zeno62/field-of-possibility/blob/main/translation/README.md">Translation Workflow</a>
- <a href="https://github.com/Zeno62/field-of-possibility/discussions">GitHub Discussions</a>
- <a href="https://zeno62.github.io/field-of-possibility/glossary">Glossary</a>

---

## Next Steps

Planned English work:

1. refine key terminology;
2. create an English abstract;
3. translate the preface and core framework chapters;
4. review conceptual consistency;
5. release an English draft version.

---

## Back

- <a href="https://zeno62.github.io/field-of-possibility/">Home</a>
- <a href="https://zeno62.github.io/field-of-possibility/zh-Hans/">Chinese Source Edition</a>