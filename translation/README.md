# Translation Workflow  
## Field of Possibility / 可能性网络

This document explains how translations of **Field of Possibility / 可能性网络** should be prepared, reviewed, and maintained.

The Chinese edition remains the source edition.  
All translations should be based on the Chinese source edition.

---

## Source Edition

The current official source edition is:

**可能性网络白皮书 v4.0 正式版**

Chinese source edition:

<a href="https://zeno62.github.io/field-of-possibility/zh-Hans/">https://zeno62.github.io/field-of-possibility/zh-Hans/</a>

Full Chinese Markdown source:

<a href="https://github.com/Zeno62/field-of-possibility/blob/main/whitepaper/zh-Hans/full/field-of-possibility-whitepaper-v4.0-zh-Hans.md">field-of-possibility-whitepaper-v4.0-zh-Hans.md</a>

Release v4.0.0:

<a href="https://github.com/Zeno62/field-of-possibility/releases/tag/v4.0.0">https://github.com/Zeno62/field-of-possibility/releases/tag/v4.0.0</a>

---

## Current English Draft Status

The English draft chapter set currently includes chapters **0–14**.

Status:

**Draft preparation**

English chapter index:

<a href="https://zeno62.github.io/field-of-possibility/en/chapters/">https://zeno62.github.io/field-of-possibility/en/chapters/</a>

English draft status note:

<a href="https://zeno62.github.io/field-of-possibility/en/draft-status">https://zeno62.github.io/field-of-possibility/en/draft-status</a>

These English chapters are introductory adaptations based on the Chinese source edition.  
They are not yet the final official English release.

---

## Translation Principles

Translations should follow these principles:

- preserve the Chinese edition as the source edition;
- translate concepts carefully rather than mechanically;
- protect the strict two-layer model;
- maintain the distinction between Awareness and consciousness;
- maintain the distinction between Pure Possibility and ordinary possibility;
- avoid turning the framework into religion, self-help, or mystical doctrine;
- avoid making scientific claims that the source text does not make;
- preserve conceptual precision over literary fluency when necessary;
- mark uncertain terms for discussion rather than silently changing them.

---

## Core Terms

Before translating or revising any chapter, check the translation map:

<a href="https://github.com/Zeno62/field-of-possibility/blob/main/glossary/translation-map.md">https://github.com/Zeno62/field-of-possibility/blob/main/glossary/translation-map.md</a>

Current key terms include:

| Chinese | English |
|---|---|
| 可能性网络 | Field of Possibility |
| 纯粹可能性 | Pure Possibility |
| 觉 | Awareness |
| 明性 | Luminosity |
| 全知 | All-Knowing |
| 可能性母网 | Field of Possibility / Total Manifestation |
| 意识 | Consciousness |
| 权重 | Weight |
| 个体节点 | Individual Node |
| 共业 / 集体权重 | Collective Weight |
| 认假为真 | Mistaking the Apparent for the Real |
| 单帧中断 | Single-Frame Interruption |
| 回归 | Return as Recognition |

Glossary:

<a href="https://zeno62.github.io/field-of-possibility/glossary">https://zeno62.github.io/field-of-possibility/glossary</a>

---

## Translation Status Labels

Each translated chapter may use one of the following labels:

- **draft** — initial translation or adaptation, not yet reviewed;
- **reviewed** — reviewed by contributors, but not yet final;
- **official** — accepted as an official translation;
- **outdated** — source version has changed and the translation needs updating.

The current English chapters are marked as:

**draft**

---

## Suggested Review Process

For each translated chapter, reviewers may check:

1. **Conceptual accuracy**

   Does the translation preserve the meaning of the Chinese source?

2. **Terminology consistency**

   Are key terms translated consistently with the translation map?

3. **Two-layer clarity**

   Does the translation preserve the distinction between source layer and manifestation layer?

4. **Tone**

   Does the text avoid becoming religious, self-help, motivational, or mystical in a vague way?

5. **Readability**

   Is the English readable for international readers without flattening the concepts?

6. **Structural clarity**

   Are sections clear enough for readers to follow the framework step by step?

---

## How to Suggest Translation Improvements

Translation suggestions may be submitted through:

GitHub Discussions:

<a href="https://github.com/Zeno62/field-of-possibility/discussions">https://github.com/Zeno62/field-of-possibility/discussions</a>

Issues:

<a href="https://github.com/Zeno62/field-of-possibility/issues">https://github.com/Zeno62/field-of-possibility/issues</a>

Pull requests:

<a href="https://github.com/Zeno62/field-of-possibility/pulls">https://github.com/Zeno62/field-of-possibility/pulls</a>

When suggesting changes, please include:

- the chapter or file name;
- the original sentence or term;
- the suggested revision;
- the reason for the change;
- whether the suggestion is about accuracy, readability, terminology, or tone.

---

## AI-Assisted Translation

AI tools may be used to assist translation, editing, comparison, and terminology review.

However:

- AI output should not be treated as final authority;
- AI may flatten or spiritualize terms;
- AI may introduce familiar but inaccurate language;
- AI may miss distinctions that are central to the framework;
- human review remains necessary.

AI can assist the translation process, but the Chinese source edition remains the source of truth.

---

## Future Translation Work

Future translation work may include:

- reviewing the English draft chapters 0–14;
- refining English terminology;
- preparing an official English release;
- creating additional multilingual editions;
- translating the glossary into more languages;
- building contributor review workflows;
- maintaining version alignment with future Chinese source editions.

---

## Related Pages

Project home:

<a href="https://zeno62.github.io/field-of-possibility/">https://zeno62.github.io/field-of-possibility/</a>

English edition home:

<a href="https://zeno62.github.io/field-of-possibility/en/">https://zeno62.github.io/field-of-possibility/en/</a>

English chapters:

<a href="https://zeno62.github.io/field-of-possibility/en/chapters/">https://zeno62.github.io/field-of-possibility/en/chapters/</a>

English draft status:

<a href="https://zeno62.github.io/field-of-possibility/en/draft-status">https://zeno62.github.io/field-of-possibility/en/draft-status</a>

Translation map:

<a href="https://github.com/Zeno62/field-of-possibility/blob/main/glossary/translation-map.md">https://github.com/Zeno62/field-of-possibility/blob/main/glossary/translation-map.md</a>